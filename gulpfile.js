...
gulp.task('styles', function() {
	return gulp.src('src/wp-content/themes/kalyan/css/**/*.css')
	.pipe(sass({ outputStyle: 'expand' }).on("error", notify.onError()))
	//.pipe(rename({ suffix: '.min', prefix : '' }))
	.pipe(concat('style.min.css'))
	.pipe(autoprefixer(['last 15 versions']))
	.pipe(cleancss( {level: { 1: { specialComments: 0 } } })) // Opt., comment out when debugging
	.pipe(gulp.dest('src/wp-content/themes/kalyan/assets/css'))
...
gulp.task('scripts', function() {
	return gulp.src([
		'src/wp-content/themes/kalyan/js/**/*.js',
		], {allowEmpty: true})
	.pipe(concat('scripts.min.js'))
	.pipe(uglify()) // Mifify js (opt.)
	.pipe(gulp.dest('src/wp-content/themes/kalyan/assets/js'))
...
gulp.task('imgmin-theme', function() {
	return gulp.src('src/wp-content/themes/kalyan/images/**/*')
	.pipe(cache(imagemin())) // Cache Images
	.pipe(gulp.dest('src/wp-content/themes/kalyan/assets/images'));
});
...
gulp.task('deploy-theme', function() {
	...
	var globs = [
	'src/wp-content/themes/kalyan/**', // Путь до шаблона у вас на компьютере
	];
	return gulp.src(globs, {buffer: false})
	.pipe(conn.dest('/www/domain.com/wp-content/themes/kalyan/')); // Путь до шаблона на хостинге
});
...
if (gulpversion == 3) {
  gulp.task('watch', ['styles', 'scripts', 'browser-sync'], function() {
	  gulp.watch(['src/wp-content/themes/kalyan/assets/sass/**/*.sass'], ['styles']); // Наблюдение за sass файлами в папке sass в теме
	  gulp.watch(['src/wp-content/themes/kalyan/assets/libs/**/*.js', 'src/wp-content/themes/kalyan/assets/js/common.js'], ['scripts']); // Наблюдение за JS файлами js в теме
    gulp.watch('src/wp-content/themes/kalyan/**/*.php', browsersync.reload) // Наблюдение за sass файлами php в теме
  });
...
if (gulpversion == 4) {
	gulp.task('watch', function() {
		gulp.watch('src/wp-content/themes/kalyan/css/**/*.css', gulp.parallel('styles')); // Наблюдение за sass файлами в папке sass в теме
		gulp.watch('src/wp-content/themes/kalyan/js/**/*.js', gulp.parallel('scripts')); // Наблюдение за JS файлами в папке js
    	gulp.watch('src/wp-content/themes/kalyan/**/*.php', browsersync.reload); // Наблюдение за sass файлами php в теме
		gulp.watch('src/wp-content/themes/kalyan/images/**/*', gulp.parallel('imgmin-theme')); // Наблюдение за sass файлами php в теме
	});
